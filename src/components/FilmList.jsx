...
const FilmList = () => {
  ...
  const showModal = (filmId) => {
    ...
    setTimeout(() => dispatch(changeModalActive(true)), 200);
  };
      ...
      <div className="movies">
        {filmList.films &&
          filmList.films.map((film) => (
            <div
              onClick={() => showModal(film.filmId)}
              key={film.filmId}
              className="movie-block"
            >
              <div className="movie-block__img">
                <img
                  className="movie-block__img-el"
                  src={film.posterUrl}
                  alt=""
                />
                <div className="movie-block__img--darkened"></div>
              </div>
              <div className="movie-block__text">
                {film.nameRu ? (
                  <div className="movie-block__text-title">{film.nameRu}</div>
                ) : (
                  <div className="movie-block__text-title">{film.nameEn}</div>
                )}

                {film.year ? (
                  <div className="movie-block__text-year">{film.year}</div>
                ) : (
                  ``
                )}
                ...
                {film.rating ? (
                  <div
                    className={[
                      `movie-block__text-rating`,
                      showMoviesRating(film.rating),
                    ].join(" ")}
                  >
                    {film.rating}
                  </div>
